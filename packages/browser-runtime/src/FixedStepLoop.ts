...
export interface FixedStepFrameStats extends FixedStepRenderFrame {
  ...
  clamped: boolean;
  /** Time spent executing fixed simulation steps during this frame. */
  simulationMs: number;
  /** Time spent inside the render callback during this frame. */
  renderingMs: number;
  /** Simulation and rendering work combined. */
  frameWorkMs: number;
}
...
export class FixedStepLoop {
  ...
  private readonly frame = (now: number): void => {
    ...
    try {
      ...
      const frame: FixedStepRenderFrame = {
        rawElapsedSeconds,
        elapsedSeconds,
        simulationSteps,
      ...
      this.onFrameStats?.({
        ...
        simulationMs,
        renderingMs,
        frameWorkMs: simulationMs + renderingMs,
      });
    } catch (error) {
      this.running = false;
      this.raf = 0;
