...
test("a raised shield consumes a lemon without taking damage", () => {
  ...
  assert.equal(player.projectiles.filter((p) => p.isLive).length, 0, "the shot was still consumed");
});
...
test("a bat chases the player, and gives up past its range", () => {
  ...
  assert.ok(Math.abs(player.pos.x - bat.pos.x) < gap, "and the chase actually closes the distance");
